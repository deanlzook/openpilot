...
from openpilot.tools.sim.bridge.common import World, SimulatorBridge, Camerad, W, H

class CarlaWorld(World):
  def __init__(self, world, vehicle, camerad, high_quality=False, dual_camera=False):
    import carla
    self.world = world
    ...
    def create_camera(fov, callback):
      ...
      blueprint.set_attribute('sensor_tick', str(1 / FPS))
      if not high_quality:
  ...
  def carla_image_to_rgb(self, image):
    ...
    rgb = np.ascontiguousarray(rgb[:, :, [0, 1, 2]])
    return rgb
  ...
  def tick(self, simulator_state: SimulatorState, startup=False):
    self.read_sensors(simulator_state)
    simulator_state.velocity = self.vehicle.get_velocity()
    simulator_state.valid = True
    simulator_state.steering_angle = self.vc.steer
    self.world.tick()
...
class CarlaBridge(SimulatorBridge):
  ...
  def spawn_world(self):
    camerad = Camerad(dual_camera=self.dual_camera)
    ...
    return CarlaWorld(world, vehicle, camerad, dual_camera=self.dual_camera)
